...
SELECT COUNT(*) AS NumberOfCustomers FROM Customers WHERE State = 'NY';

SELECT FullName, Email FROM Customers WHERE SubscriptionEndDate IS NULL;

CREATE INDEX idx_State ON Customers(State);

INSERT INTO Customers (FullName, Email, State, SubscriptionLevel, SubscriptionStartDate)
VALUES ('John Doe', 'john.doe@example.com', 'CA', 'Basic', '2024-07-01');

UPDATE Customers SET SubscriptionLevel = 'Premium' WHERE FullName = 'Emma Collins';

SELECT COUNT(*) AS EduEmailCount FROM Customers WHERE Email LIKE '%.edu';

DELETE FROM Customers WHERE SubscriptionEndDate < '2024-08-01';

SELECT FullName, Email FROM Customers WHERE SubscriptionLevel = 'Premium' AND State = 'NY' ORDER BY FullName;
...
VALUES 
('Pizza', 400, 11.99, 0, 'Basic'),
('Tomorrows bread', 350, 15.99, 0, 'Premium'),
('Chicken nuggies', 250, 9.99, 0, 'Basic');
...
UPDATE Meals SET Price = Price + 5.00 WHERE SubscriptionLevel = 'Premium';
